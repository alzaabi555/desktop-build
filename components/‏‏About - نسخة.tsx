...
const About: React.FC = () => {
 ...
 const { t, dir, language } = useApp();
  ...
  return (
    <div className={`flex flex-col h-full overflow-hidden transition-colors duration-500 relative z-10 ${language === 'ar' ? 'text-right' : 'text-left'} ${isRamadan ? 'text-white' : 'bg-[#fcfdfe] text-textPrimary'}`} dir={dir}>
      
      {/* ================= 🩺 الهيدر القياسي الممتد للنوتش ================= */}
      <header className={`shrink-0 z-40 px-4 pt-[env(safe-area-inset-top)] w-full transition-all duration-300 bg-transparent ${isRamadan ? 'text-white' : 'text-textPrimary'}`}>
        <div className="flex justify-between items-center max-w-2xl mx-auto w-full pb-4">
            <div className="flex items-center gap-3">
                <div className="bg-bgCard/10 p-2 rounded-xl border border-borderColor shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-2 pb-28 custom-scrollbar relative z-10">
          <div className={`flex flex-col items-center max-w-2xl mx-auto w-full animate-in fade-in zoom-in-95 duration-500`}>
              ...
              <div className="flex flex-col items-center mb-10 mt-4">
                  <div className={`w-32 h-32 rounded-[2.5rem] shadow-2xl flex items-center justify-center mb-6 border p-4 relative group hover:scale-105 transition-transform duration-500 select-none backdrop-blur-xl ${isRamadan ? 'bg-bgCard/10 border-borderColor' : 'bg-bgCard border-slate-100'}`}>
                  ...
                  <h1 className={`text-3xl font-black mb-2 tracking-tight transition-colors ${isRamadan ? 'text-white' : 'text-textPrimary'}`}>
                      {t('appNameAbout')}
                  </h1>
                  <div className={`px-4 py-1.5 rounded-full font-black text-xs shadow-sm border ${isRamadan ? 'bg-bgCard/10 border-white/10 text-indigo-200' : 'bg-slate-100 border-slate-200 text-textSecondary'}`}>
                      {t('appVersionLabel')}
                  </div>
              </div>
              ...
              <div className="w-full space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className={`rounded-3xl overflow-hidden border p-6 transition-all duration-500 ${isRamadan ? 'bg-bgCard/5 border-white/10 backdrop-blur-md' : 'bg-bgCard border-slate-100 shadow-sm'}`}>
                        <div className="flex items-center gap-4 mb-5">
                            ...
                            <div>
                                ...
                                <p className={`text-xs font-bold mt-0.5 ${isRamadan ? 'text-indigo-200/70' : 'text-textSecondary'}`}>{t('aboutDeveloperTitle')}</p>
                            </div>
                        </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className={`rounded-3xl overflow-hidden border flex flex-col divide-y transition-all duration-500 ${isRamadan ? 'bg-bgCard/5 border-white/10 divide-white/5' : 'bg-bgCard border-slate-100 divide-slate-50 shadow-sm'}`}>
                        
                        {/* الدعم عبر الهاتف */}
                        <a href="tel:98344555" className={`w-full p-4 flex items-center justify-between transition-colors active:bg-bgCard/5 ${isRamadan ? 'hover:bg-bgCard/5' : 'hover:bg-slate-50'}`}>
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex flex-col items-start">
                                    ...
                                    <span className={`text-xs font-black tracking-wider mt-0.5 ${isRamadan ? 'text-slate-400' : 'text-textSecondary'}`} dir="ltr">98344555</span>
                                </div>
                            </div>
                        ...
                        {/* البريد الإلكتروني */}
                        <a href="mailto:mohammad.alzaabi21@edu.moe.om" className={`w-full p-4 flex items-center justify-between transition-colors active:bg-bgCard/5 ${isRamadan ? 'hover:bg-bgCard/5' : 'hover:bg-slate-50'}`}>
                            <div className="flex items-center gap-4 min-w-0">
                                ...
                                <div className="flex flex-col items-start min-w-0 pr-2">
                                    ...
                                    <span className={`text-[10px] font-bold mt-0.5 truncate w-full ${isRamadan ? 'text-slate-400' : 'text-textSecondary'}`}>mohammad.alzaabi21@edu.moe.om</span>
                                </div>
                            </div>
                        ...
                        {/* Github */}
                        <a href="https://*****" target="_blank" rel="noreferrer" className={`w-full p-4 flex items-center justify-between transition-colors active:bg-bgCard/5 ${isRamadan ? 'hover:bg-bgCard/5' : 'hover:bg-slate-50'}`}>
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex flex-col items-start">
                                    ...
                                    <span className={`text-xs font-bold mt-0.5 ${isRamadan ? 'text-slate-400' : 'text-textSecondary'}`}>******</span>
                                </div>
                            </div>
                            <ExternalLink size={16} className={isRamadan ? 'text-slate-600' : 'text-slate-400'} />
                        </a>
              ...
              {/* Footer */}
              <div className={`mt-10 mb-4 flex flex-col items-center justify-center gap-1 text-[10px] font-black uppercase tracking-widest transition-colors ${isRamadan ? 'text-textSecondary' : 'text-slate-400'}`}>
