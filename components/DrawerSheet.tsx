...
const DrawerSheet: React.FC<DrawerSheetProps> = ({ 
    ...
    if (mode === 'full') {
        // 🚀 شاشة كاملة تنزلق من الأسفل (لحل مشكلة الإكسل، ستغطي شاشة الكمبيوتر والجوال بالكامل فوق كل شيء)
        positioningStyles = 'inset-0 w-full h-full rounded-none';
        transformStyles = isOpen ? 'translate-y-0' : 'translate-y-full';
    } 
    else if (mode === 'side') {
        // 🚀 لوحة جانبية منزلقة من اليمين أو اليسار (حسب اللغة)
        ...
        // 🚀 في الجوال من الأسفل، وفي الكمبيوتر تنزلق من الجانب (كما كانت في تصميمنا الأصلي)
    ...
    return (
        <>
            ...
            <div
                className={`fixed inset-0 bg-black/70 backdrop-blur-sm z-[99998] transition-opacity duration-500 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
                onClick={onClose}
            />
            
            {/* 🚀 الحاوية المنزلقة (z-index 99999 لتتجاوز القائمة الجانبية للكمبيوتر وتغطيها) */}
            <div
                className={`fixed z-[99999] flex flex-col shadow-[0_0_60px_rgba(0,0,0,0.8)] transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]
                    ${positioningStyles}
                ...
                `}
            >
                {/* 💉 المضاد الحيوي للون الأخضر داخل النوافذ المنزلقة */}
                {isRamadan && (
                    <style>
                        {`
