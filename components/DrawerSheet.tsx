...
const DrawerSheet: React.FC<DrawerSheetProps> = ({ 
    ...
    return (
        <>
            ...
            <div
                className={`fixed inset-0 bg-black/70 backdrop-blur-sm transition-opacity duration-500 z-[99998] ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
                onClick={onClose}
                // أضفنا هذا السطر لضمان أن الخلفية تظهر فوق كل شيء (حتى الـ Sidebar)
                style={{ zIndex: 99998 }} 
            />
            
            {/* الحاوية المنزلقة */}
            <div
                // أضفنا style للـ zIndex للتأكيد المطلق، وأضفنا كلاس ramadan-overrides 
                className={`fixed flex flex-col shadow-[0_0_60px_rgba(0,0,0,0.8)] transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]
                    ...
                    ${transformStyles}
                `}
                style={{ zIndex: 99999 }} // 💉 تأكيد الـ zIndex المرتفع جداً
            >
                {/* 💉 المضاد الحيوي المحسن (الشامل لكل الكلاسات الخضراء المحتملة) */}
                {isRamadan && (
                    <style>
                        {`
                            ...
                            /* 🚀 الأهم: استهداف حالات الـ Focus (التركيز) والـ Ring (الهالة) لحقول الإدخال */
                            .ramadan-overrides input:focus, 
                            .ramadan-overrides textarea:focus,
                            .ramadan-overrides select:focus {
                                border-color: rgba(168, 85, 247, 0.5) !important; /* لون بنفسجي لثيم رمضان */
                                outline: none !important;
                                box-shadow: 0 0 0 2px rgba(168, 85, 247, 0.2) !important; 
                                ring-color: rgba(168, 85, 247, 0.5) !important; /* تجاوز أي ring لـ Tailwind */
                            }

                            /* استهداف أي عنصر يحتوي على كلاس ring-green */
                            .ramadan-overrides [class*="ring-green"] {
                                --tw-ring-color: rgba(168, 85, 247, 0.5) !important;
                            }
                            .ramadan-overrides [class*="focus:ring-green"]:focus {
                                --tw-ring-color: rgba(168, 85, 247, 0.5) !important;
                            }
                        `}
                    </style>
