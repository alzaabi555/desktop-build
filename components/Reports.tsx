...
const Reports: React.FC = () => {
            ...
            <div style="page-break-after: always; padding: 40px; font-family: 'Tajawal', sans-serif; direction: rtl; background: white; color: black; box-sizing: border-box; height: 100vh; position: relative;">
                ...
                <div style="background:#f8fafc; border:1px solid #cbd5e1; padding:15px; border-radius:10px; margin-bottom:20px; display:flex; justify-content:space-between; align-items:center;">
                    <div style="flex:1;">
                        ...
                        <div style="display:flex; gap:10px;">
                            <span style="background:#dcfce7; color:#000; padding:2px 8px; border-radius:4px; font-size:10px; font-weight:bold;">إيجابي: ${totalPositive}</span>
                            <span style="background:#ffe4e6; color:#000; padding:2px 8px; border-radius:4px; font-size:10px; font-weight:bold;">سلبي: ${totalNegative}</span>
                        </div>
                    </div>
                </div>
                ...
                <div style="display:flex; gap:15px; margin-bottom:20px;">
                    <div style="flex:1; border:1px solid #cbd5e1; padding:10px; border-radius:8px; text-align:center;">
                        ...
                        <span style="font-weight:900; color:#000; font-size:18px;">${absenceCount}</span>
                    </div>
                    <div style="flex:1; border:1px solid #cbd5e1; padding:10px; border-radius:8px; text-align:center;">
                        <span style="display:block; font-size:10px; color:#000;">الهروب (التسرب)</span>
                        <span style="font-weight:900; color:#000; font-size:18px;">${truantCount}</span>
                    </div>
                </div>
  ...
  const handlePrintGradeReport = async () => {
      ...
      continuousTools.forEach(t => {
          headerHtml += `<th style="background-color:#ffedd5 !important;">${t.name}</th>`;
      });

      headerHtml += `
        <th style="width:60px; background-color:#dbeafe !important; border-right: 2px solid #000 !important;">المجموع (60)</th>
      `;

      if (finalTool) {
          headerHtml += `<th style="width:70px; background-color:#fce7f3 !important;">${finalTool.name} (40)</th>`;
      }

      headerHtml += `
        <th style="width:60px; background-color:#e5e7eb !important;">المجموع الكلي</th>
      ...
      const html = `
        <div style="padding:20px; font-family:'Tajawal', sans-serif; width:100%; direction:rtl; color:black; background:white;">
            <style>
              table { width: 100%; border-collapse: collapse; margin-top: 10px; border: 2px solid #000; font-size: 10px; color: black; }
              th { background-color: #e5e7eb; font-weight: bold; padding: 6px; border: 1px solid #000; color: black; }
              td { padding: 4px; border: 1px solid #000; text-align: center; color: black; }
            ...
            <div style="margin-top:30px; display:flex; justify-content:space-between; padding:0 40px; font-weight:bold;">
                <p>توقيع المعلم: .................</p>
  ...
  const printCertificates = async () => {
      ...
      targets.forEach((s) => {
          ...
          let body = certificateSettings.bodyText.replace(placeholderRegex, `<span style="font-weight:900;">${s.name}</span>`);
          ...
          const headerHtml = `
            <div style="width:100%; text-align:center; display:flex; flex-direction:column; align-items:center; margin-bottom:5px;">
          ...
          pagesHtml += `
            <div style="width:100%; height:100vh; position:relative; ${bgStyle} padding:20px; box-sizing:border-box; display:flex; flex-direction:column; align-items:center; text-align:center; page-break-after: always; background-color:#fff; color: black;">
                ...
                <div style="z-index:10; width:95%; height:100%; display:flex; flex-direction:column; justify-content:flex-start; background:rgba(255,255,255,0.92); padding:30px; padding-top:10px; border-radius:30px; box-shadow:none;">
                    ${headerHtml}
                    <div style="flex:1; display:flex; flex-direction:column; justify-content:center; align-items:center; margin-top:-20px;">
                        ...
                        ${!hasPlaceholder ? `<h2 style="font-size:42px; margin:10px 0 20px 0; font-weight:900; text-decoration:underline; text-decoration-color:#059669; text-underline-offset: 8px;">${s.name}</h2>` : ''}

                        <p style="font-size:24px; line-height:1.6; margin-bottom:20px; font-weight:bold; color:#374151 !important; max-width:80%;">${body}</p>
                    </div>
                    
                    <div style="margin-top:20px; display:flex; justify-content:space-between; width:100%; padding:0 20px; align-items:flex-end;">
                        <div style="text-align:center; width:30%;">
                            <p style="font-size:18px; margin-bottom:40px; font-weight:bold;">معلم المادة</p>
                            <p style="font-size:20px; font-weight:900;">${teacherInfo.name}</p>
                        </div>
                        ...
                        <div style="text-align:center; width:30%;">
                            <p style="font-size:18px; margin-bottom:40px; font-weight:bold;">مدير المدرسة</p>
  ...
  const handlePrintSummon = async () => {
      ...
      const html = `
        <div style="padding:50px; font-family:'Tajawal', serif; direction:rtl; text-align:right; width:100%; height:100%; background: white; color: black;">
            <div style="text-align:center; margin-bottom:40px;">
                ...
                <h3 style="font-weight:bold; font-size:16px; margin:5px;">سلطنة عمان</h3>
                <h3 style="font-weight:bold; font-size:16px; margin:5px;">وزارة التربية والتعليم</h3>
                <h3 style="font-weight:bold; font-size:16px; margin:5px;">مدرسة ${teacherInfo.school}</h3>
            </div>
            
            <div style="border-bottom:2px solid #000; padding-bottom:20px; margin-bottom:30px;">
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <span style="font-weight:bold; font-size:18px;">الفاضل/ ولي أمر الطالب: ${studentName}</span>
                    <span style="font-size:18px;">المحترم</span>
                </div>
                <div style="display:flex; justify-content:space-between;">
                    <span style="font-weight:bold; font-size:16px;">الصف: ${summonClass}</span>
                    <span style="font-size:16px;">التاريخ: ${issueDate}</span>
                </div>
            </div>

            <h2 style="text-align:center; font-weight:900; font-size:24px; text-decoration:underline; margin-bottom:30px;">استدعاء ولي أمر</h2>
            
            <p style="line-height:2.2; font-size:18px; text-align:justify; margin-bottom:20px;">
            ...
            <div style="background:#f9f9f9; border:1px solid #000; padding:20px; text-align:center; font-size:20px; font-weight:bold; margin-bottom:30px; border-radius:10px;">
                ${getReasonText()}
            </div>

            ${proceduresHtml}
            
            <p style="line-height:2; font-size:18px; text-align:justify; margin-top:30px; margin-bottom:50px;">
                شاكرين لكم حسن تعاونكم واهتمامكم بمصلحة الطالب.
            </p>
            
            <div style="display:flex; justify-content:space-between; margin-top:80px; padding:0 20px; align-items:flex-end; position:relative;">
                <div style="text-align:center; width:30%;">
                    <p style="font-weight:bold; font-size:18px; margin-bottom:40px;">معلم المادة</p>
                    <p style="font-size:18px;">${teacherInfo.name}</p>
                </div>
                ...
                <div style="text-align:center; width:30%;">
                    <p style="font-weight:bold; font-size:18px; margin-bottom:40px;">مدير المدرسة</p>
  ...
  const tabItems = [
    { id: 'student_report', label: 'تقرير طالب', icon: User, color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-100' },
    { id: 'grades_record', label: 'سجل الدرجات', icon: BarChart3, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-100' },
    { id: 'certificates', label: 'الشهادات', icon: Award, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100' },
    { id: 'summon', label: 'استدعاء ولي أمر', icon: FileWarning, color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100' },
  ];
  ...
  return (
    <div className="flex flex-col w-full max-w-5xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4 pt-4 px-2 mb-2">
        <div className="w-14 h-14 bg-white border border-gray-200 rounded-2xl flex items-center justify-center text-rose-500 shadow-sm"><FileSpreadsheet size={30} /></div>
        <div><h2 className="text-3xl font-black text-slate-800 tracking-tight">مركز التقارير</h2><p className="text-gray-500 text-xs font-bold mt-1">طباعة الكشوفات والشهادات والاستدعاءات</p></div>
      </div>

      <div className="grid grid-cols-2 gap-3 px-1">
        {tabItems.map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id as any)} className={`flex items-center gap-4 p-4 rounded-[1.5rem] transition-all duration-300 border group shimmer-hover ${activeTab === item.id ? 'bg-white border-indigo-200 shadow-lg scale-[1.02]' : 'bg-white/50 border-gray-100 opacity-80 hover:opacity-100'}`}>
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-all duration-500 group-hover:scale-110 ${activeTab === item.id ? 'bg-indigo-600 text-white shadow-md' : `${item.bg} ${item.color} border ${item.border}`}`}><item.icon size={24} strokeWidth={2} /></div>
                <div className="text-right"><span className={`block font-black text-sm transition-colors ${activeTab === item.id ? 'text-slate-800' : 'text-gray-500 group-hover:text-slate-800'}`}>{item.label}</span></div>
            </button>
        ))}
      </div>

      <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-gray-100 min-h-[400px] shadow-sm relative overflow-hidden transition-all duration-500">
        {activeTab === 'student_report' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                 <div className="pb-4 border-b border-gray-100 flex items-center gap-3"><div className="p-2 bg-indigo-50 rounded-xl text-indigo-600"><User size={20}/></div><div><h3 className="font-black text-lg text-slate-800">تقرير الطالب الشامل</h3><p className="text-gray-500 text-xs font-bold">عرض وطباعة تقرير مفصل</p></div></div>
                
                <div className="space-y-4">
                    {availableGrades.length > 0 && (
                        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                            <button onClick={() => setStGrade('all')} className={`px-4 py-1.5 text-[10px] font-bold whitespace-nowrap rounded-xl transition-all border ${stGrade === 'all' ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-gray-50 border-gray-200 text-gray-500'}`}>كل المراحل</button>
                            {availableGrades.map(g => (
                                <button key={g} onClick={() => setStGrade(g)} className={`px-4 py-1.5 text-[10px] font-bold whitespace-nowrap rounded-xl transition-all border ${stGrade === g ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-gray-50 border-gray-200 text-gray-500'}`}>صف {g}</button>
                            ))}
                        </div>
                    )}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 mr-2">الفصل الدراسي</label><div className="relative"><select value={stClass} onChange={(e) => setStClass(e.target.value)} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-bold outline-none appearance-none cursor-pointer text-slate-800">{getClassesForGrade(stGrade).map(c => <option key={c} value={c}>{c}</option>)}</select><ChevronDown className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" /></div></div>
                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 mr-2">الطالب</label><div className="relative"><select value={selectedStudentId} onChange={(e) => setSelectedStudentId(e.target.value)} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-bold outline-none appearance-none cursor-pointer text-slate-800"><option value="">اختر طالباً...</option>{filteredStudentsForStudentTab.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select><ChevronDown className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" /></div></div>
                    </div>
                </div>

                <div className="flex flex-col md:flex-row gap-4 pt-6 justify-end border-t border-gray-100 mt-4">
                    <button onClick={handlePrintClassReports} disabled={isGeneratingPdf || !stClass} className="bg-gray-800 text-white border border-gray-600 px-6 py-4 rounded-2xl font-black text-xs flex items-center justify-center gap-2 shadow-lg hover:bg-gray-900 active:scale-95 transition-all">
                        {isGeneratingPdf ? <Loader2 className="animate-spin w-4 h-4"/> : <Layers size={18} />} طباعة تقارير الفصل ({stClass})
                    </button>
                    <button onClick={handleViewStudentReport} disabled={!selectedStudentId} className="bg-indigo-600 disabled:opacity-50 text-white px-8 py-4 rounded-2xl font-black text-xs flex items-center justify-center gap-2 shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all">
        ...
        {activeTab === 'grades_record' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="pb-4 border-b border-gray-100 flex items-center gap-3"><div className="p-2 bg-amber-50 rounded-xl text-amber-600"><BarChart3 size={20}/></div><div><h3 className="font-black text-lg text-slate-800">سجل الدرجات</h3><p className="text-gray-500 text-xs font-bold">طباعة كشف درجات كامل</p></div></div>
                <div className="space-y-4">
                    {availableGrades.length > 0 && (
                        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                            <button onClick={() => { setGradesGrade('all'); setGradesClass('all'); }} className={`px-4 py-1.5 text-[10px] font-bold whitespace-nowrap rounded-xl transition-all border ${gradesGrade === 'all' ? 'bg-amber-500 text-white border-amber-600' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>كل المراحل</button>
                            {availableGrades.map(g => (
                                <button key={g} onClick={() => { setGradesGrade(g); setGradesClass('all'); }} className={`px-4 py-1.5 text-[10px] font-bold whitespace-nowrap rounded-xl transition-all border ${gradesGrade === g ? 'bg-amber-500 text-white border-amber-600' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>صف {g}</button>
                            ))}
                        </div>
                    )}
                    <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 mr-2">الفصل الدراسي</label><div className="relative"><select value={gradesClass} onChange={(e) => setGradesClass(e.target.value)} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-bold outline-none appearance-none cursor-pointer text-slate-800"><option value="all">جميع الفصول</option>{getClassesForGrade(gradesGrade).map(c => <option key={c} value={c}>{c}</option>)}</select><ChevronDown className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" /></div></div>
                </div>
                <div className="flex justify-end pt-6"><button onClick={handlePrintGradeReport} disabled={isGeneratingPdf} className="bg-amber-500 disabled:opacity-50 text-white px-8 py-4 rounded-2xl font-black text-xs flex items-center gap-2 shadow-lg shadow-amber-200 hover:bg-amber-600 active:scale-95 transition-all w-full md:w-auto justify-center">{isGeneratingPdf ? <Loader2 className="animate-spin w-4 h-4"/> : <Printer size={18} />} طباعة السجل</button></div>
            </div>
        )}
        
        {activeTab === 'certificates' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex justify-between items-start pb-4 border-b border-gray-100"><div className="flex items-center gap-3"><div className="p-2 bg-emerald-50 rounded-xl text-emerald-600"><Award size={20}/></div><div><h3 className="font-black text-lg text-slate-800">شهادات التفوق</h3><p className="text-gray-500 text-xs font-bold">طباعة شهادات تقدير</p></div></div><button onClick={() => setShowCertSettingsModal(true)} className="p-3 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors text-slate-600"><Settings className="w-5 h-5" /></button></div>
                <div className="grid grid-cols-1 gap-6">
                    <div className="space-y-4">
                        {availableGrades.length > 0 && (
                            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                                <button onClick={() => setCertGrade('all')} className={`px-4 py-1.5 text-[10px] font-bold whitespace-nowrap rounded-xl transition-all border ${certGrade === 'all' ? 'bg-emerald-600 text-white border-emerald-700' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>كل المراحل</button>
                                {availableGrades.map(g => (
                                    <button key={g} onClick={() => setCertGrade(g)} className={`px-4 py-1.5 text-[10px] font-bold whitespace-nowrap rounded-xl transition-all border ${certGrade === g ? 'bg-emerald-600 text-white border-emerald-700' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>صف {g}</button>
                                ))}
                            </div>
                        )}
                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 mr-2">الفصل الدراسي</label><div className="relative"><select value={certClass} onChange={(e) => { setCertClass(e.target.value); setSelectedCertStudents([]); }} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-bold outline-none appearance-none cursor-pointer text-slate-800">{getClassesForGrade(certGrade).map(c => <option key={c} value={c}>{c}</option>)}</select><ChevronDown className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" /></div></div>
                    </div>
                    <div className="space-y-2"><div className="flex justify-between items-center px-2"><label className="text-[10px] font-black text-gray-400">تحديد الطلاب ({selectedCertStudents.length})</label><button onClick={selectAllCertStudents} className="text-[10px] text-emerald-600 font-bold hover:underline">{selectedCertStudents.length === filteredStudentsForCert.length ? 'إلغاء الكل' : 'تحديد الكل'}</button></div><div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-60 overflow-y-auto custom-scrollbar p-1">{filteredStudentsForCert.map(s => (<button key={s.id} onClick={() => toggleCertStudent(s.id)} className={`p-3 rounded-xl border text-[10px] font-bold transition-all flex items-center justify-between group ${selectedCertStudents.includes(s.id) ? 'bg-emerald-600 text-white border-emerald-500 shadow-md' : 'bg-white text-slate-600 border-gray-200 hover:bg-gray-50'}`}>{s.name}{selectedCertStudents.includes(s.id) && <Check size={14} className="bg-white/20 rounded-full p-0.5" />}</button>))}</div></div>
                </div>
                <div className="flex justify-end pt-6 border-t border-gray-100 mt-2"><button onClick={printCertificates} disabled={isGeneratingPdf || selectedCertStudents.length === 0} className="bg-emerald-600 disabled:opacity-50 text-white px-8 py-4 rounded-2xl font-black text-xs flex items-center gap-2 shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-95 transition-all w-full md:w-auto justify-center">{isGeneratingPdf ? <Loader2 className="animate-spin w-4 h-4"/> : <Award size={18} />} طباعة الشهادات</button></div>
            </div>
        )}
        
        {activeTab === 'summon' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="pb-4 border-b border-gray-100 flex items-center gap-3"><div className="p-2 bg-rose-50 rounded-xl text-rose-600"><FileWarning size={20}/></div><div><h3 className="font-black text-lg text-slate-800">استدعاء ولي أمر</h3><p className="text-gray-500 text-xs font-bold">إنشاء خطاب رسمي</p></div></div>
                <div className="space-y-4">
                    {availableGrades.length > 0 && (
                        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                            <button onClick={() => setSummonGrade('all')} className={`px-4 py-1.5 text-[10px] font-bold whitespace-nowrap rounded-xl transition-all border ${summonGrade === 'all' ? 'bg-rose-600 text-white border-rose-700' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>كل المراحل</button>
                            {availableGrades.map(g => (
                                <button key={g} onClick={() => setSummonGrade(g)} className={`px-4 py-1.5 text-[10px] font-bold whitespace-nowrap rounded-xl transition-all border ${summonGrade === g ? 'bg-rose-600 text-white border-rose-700' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>صف {g}</button>
                            ))}
                        </div>
                    )}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 mr-2">الفصل</label><div className="relative"><select value={summonClass} onChange={(e) => setSummonClass(e.target.value)} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-bold outline-none appearance-none cursor-pointer text-slate-800">{getClassesForGrade(summonGrade).map(c => <option key={c} value={c}>{c}</option>)}</select><ChevronDown className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" /></div></div>
                        <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 mr-2">الطالب</label><div className="relative"><select value={summonStudentId} onChange={(e) => setSummonStudentId(e.target.value)} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-bold outline-none appearance-none cursor-pointer text-slate-800"><option value="">اختر...</option>{availableStudentsForSummon.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select><ChevronDown className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" /></div></div>
                    </div>
                </div>
                <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 mr-2">سبب الاستدعاء</label><div className="flex flex-wrap gap-2">{[{ id: 'absence', label: 'تكرار الغياب' }, { id: 'truant', label: 'تسرب حصص' }, { id: 'behavior', label: 'سلوكيات' }, { id: 'level', label: 'تدني مستوى' }, { id: 'other', label: 'آخر ..' }].map((reason) => (<button key={reason.id} onClick={() => setReasonType(reason.id)} className={`px-4 py-2.5 rounded-xl text-[10px] font-black transition-all border ${reasonType === reason.id ? 'bg-rose-600 text-white border-rose-600 shadow-md transform scale-105' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}>{reason.label}</button>))}</div>{reasonType === 'other' && (<textarea value={customReason} onChange={(e) => setCustomReason(e.target.value)} placeholder="اكتب السبب هنا..." className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm mt-2 resize-none h-24 outline-none focus:border-rose-500 transition-colors text-slate-800" />)}</div>
                <div className="space-y-2 border-t border-gray-100 pt-4"><label className="text-[10px] font-black text-gray-400 mr-2 flex items-center gap-2"><ListChecks className="w-3 h-3" /> الإجراءات المتخذة مسبقاً</label><div className="grid grid-cols-2 gap-2">{availableProcedures.map((proc) => (<button key={proc} onClick={() => toggleProcedure(proc)} className={`p-3 rounded-xl text-[10px] font-bold text-right transition-all border flex items-center justify-between ${takenProcedures.includes(proc) ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-gray-100 text-gray-500 hover:bg-gray-50'}`}>{proc}{takenProcedures.includes(proc) && <Check className="w-3 h-3 text-indigo-600" />}</button>))}</div></div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                     <div className="space-y-1"><label className="text-[10px] font-black text-gray-400 mr-2">تاريخ الإصدار</label><input type="date" value={issueDate} onChange={(e) => setIssueDate(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold outline-none text-slate-800" /></div>
                     <div className="space-y-1"><label className="text-[10px] font-black text-gray-400 mr-2">تاريخ الحضور</label><input type="date" value={summonDate} onChange={(e) => setSummonDate(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold outline-none text-slate-800" /></div>
                     <div className="space-y-1"><label className="text-[10px] font-black text-gray-400 mr-2">وقت الحضور</label><input type="time" value={summonTime} onChange={(e) => setSummonTime(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold outline-none text-slate-800" /></div>
                </div>
                <div className="flex gap-4 pt-6 border-t border-gray-100 mt-2"><button onClick={() => setShowSummonPreview(true)} disabled={!summonStudentId} className="flex-1 py-4 bg-gray-800 border border-gray-600 text-white rounded-2xl font-black text-xs shadow-lg hover:bg-gray-900 transition-all disabled:opacity-50 flex items-center justify-center gap-2"><Eye size={18} /> معاينة الخطاب</button><button onClick={handleSendSummonWhatsApp} disabled={!summonStudentId} className="flex-1 py-4 bg-[#25D366] text-white rounded-2xl font-black text-xs shadow-lg shadow-green-200 hover:bg-[#128C7E] transition-all disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95"><Share2 size={18} /> إرسال واتساب</button></div>
            </div>
        )}
      </div>

      {showSummonPreview && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setShowSummonPreview(false)}>
            <div className="bg-white w-full max-w-4xl h-[90vh] rounded-2xl flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b flex justify-between items-center text-slate-900 bg-gray-50"><h3 className="font-bold">معاينة الخطاب</h3><div className="flex gap-2"><button onClick={handlePrintSummon} className="p-2 bg-indigo-600 text-white rounded-lg flex items-center gap-2 text-xs font-bold shadow-md hover:bg-indigo-700"><Printer size={16} /> طباعة PDF</button><button onClick={() => setShowSummonPreview(false)} className="p-2 hover:bg-gray-200 rounded-full text-gray-500"><X size={24}/></button></div></div>
                <div className="flex-1 overflow-auto bg-gray-100 p-8 flex justify-center">
                    <div className="bg-white text-black w-[210mm] min-h-[297mm] p-[20mm] text-right font-serif relative border border-gray-300 shadow-sm box-border">
      ...
      <Modal isOpen={showCertSettingsModal} onClose={() => setShowCertSettingsModal(false)} className="max-w-md rounded-[2rem]">
          <div className="text-center"><h3 className="font-black text-lg mb-4 text-slate-800">إعدادات الشهادة</h3><div className="space-y-3"><div><label className="block text-xs font-bold text-gray-500 mb-1 text-right">عنوان الشهادة</label><input type="text" value={tempCertSettings.title} onChange={(e) => setTempCertSettings({...tempCertSettings, title: e.target.value})} className="w-full p-3 bg-white border border-gray-200 rounded-xl text-sm font-bold outline-none text-slate-800" /></div><div><label className="block text-xs font-bold text-gray-500 mb-1 text-right">نص الشهادة</label><textarea value={tempCertSettings.bodyText} onChange={(e) => setTempCertSettings({...tempCertSettings, bodyText: e.target.value})} className="w-full p-3 bg-white border border-gray-200 rounded-xl text-sm font-bold outline-none h-24 resize-none text-slate-800" /></div><div className="flex gap-2 mt-4 pt-2"><button onClick={() => setShowCertSettingsModal(false)} className="flex-1 py-3 text-gray-500 font-bold text-xs hover:bg-gray-100 rounded-xl">إلغاء</button><button onClick={handleSaveCertSettings} className="flex-[2] py-3 bg-indigo-600 text-white rounded-xl font-black text-sm shadow-lg">حفظ الإعدادات</button></div></div></div>
      </Modal>
    </div>
  );
};
