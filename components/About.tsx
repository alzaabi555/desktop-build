...
const About: React.FC = () => {
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-full p-6 animate-in fade-in zoom-in duration-500 relative z-10 transition-colors ${isRamadan ? 'text-white' : 'text-slate-900'}`}>
      ...
      {/* Logo Container */}
      <div className={`w-40 h-40 glass-heavy rounded-[3rem] shadow-2xl flex items-center justify-center mb-8 border p-4 relative group hover:scale-105 transition-all select-none ${isRamadan ? 'bg-white/10 border-white/20' : 'bg-white border-white/20'}`}>
      ...
      <p className={`glass-card px-6 py-2 rounded-full font-black text-xs mb-10 border-none shadow-sm transition-colors ${isRamadan ? 'text-indigo-200 bg-white/10' : 'text-slate-500 bg-white'}`}>V 4.1.7 • الإصدار الرمضاني</p>
      
      <div className={`glass-heavy border rounded-[3rem] p-10 max-w-md w-full text-center transition-all duration-500 ${isRamadan ? 'bg-[#0f172a]/60 backdrop-blur-2xl border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.5)]' : 'bg-white/80 backdrop-blur-xl border-white/20 shadow-xl'}`}>
          ...
          <div className="flex flex-col gap-4">
              <div className={`flex items-center gap-5 p-5 glass-card rounded-[2rem] transition-transform duration-300 hover:-translate-y-1 border shadow-sm ${isRamadan ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-white border-gray-100 hover:bg-gray-50'}`}>
