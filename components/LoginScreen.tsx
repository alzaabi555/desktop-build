...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess }) => {
  ...
  // لمنع تنفيذ onLoginSuccess مرتين
  const completedRef = useRef(false);

  // ✅ Listen to deep links only in Electron
  useEffect(() => {
    ...
    const unsubscribe = api.onDeepLink((url: string) => {
      ...
      // هنا لاحقًا سنعمل:
      // 1) نتحقق أن الرابط خاص بتسجيل الدخول (مثلاً rasedapp://auth?code=...)
      // 2) نرسل url إلى main أو إلى backend لتبادل code ثم نستخدم signInWithCustomToken
      // الآن مؤقت: نعتبر أن وصول الرابط = نجاح تدفق العودة
      ...
      // ✅ مؤقت: مرّر قيمة رمزية بدل user الحقيقي
      // في الخطوة القادمة سنستبدله بمستخدم Firebase الحقيقي.
      onLoginSuccess({ provider: 'google', platform: 'electron', deepLinkUrl: url });
    });
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      // ✅ على Electron لا نستخدم Firebase Web popup إطلاقًا
      if (isElectron) {
        // حاليا: افتح صفحة تسجيل Google خارجيًا يدويًا (مؤقت)
        // في الخطوة القادمة سنستبدله بـ: window.electron.startGoogleLogin()
        // وسنجعل main يبني رابط OAuth الصحيح ويفتحه بالمتصفح.
        const msg =
          'نسخة Windows تحتاج تدفق OAuth خارجي + Deep Link.\n' +
          'الآن أرسل لي رابط OAuth الذي تريد فتحه أو دعنا نضيف startGoogleLogin في main.';
        console.log(msg);

        // لإبقاء الواجهة في وضع تحميل حتى يأتي deep link
        // يمكنك مؤقتًا إيقاف التحميل إذا أحببت:
        // setIsLoading(false);

        // تنبيه للمطور (يمكن حذفه لاحقًا)
        alert('Windows: بانتظار إعداد OAuth الخارجي (الخطوة القادمة).');

        return;
      }

      // ✅ للويب/الموبايل: استخدم الطريقة الحالية
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F2F2F7] relative overflow-hidden">
      ...
      <div
        className="absolute bottom-[-20%] right-[-20%] w-[80vw] h-[80vw] bg-rose-500/20 rounded-full blur-[120px] pointer-events-none animate-pulse"
