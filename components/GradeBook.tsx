...
const GradeBook: React.FC<GradeBookProps> = ({ 
  ...
  const styles = {
      card: `
        ...
        hover:border-indigo-300 hover:shadow-[0_8px_20px_-4px_rgba(79,70,229,0.1)] hover:-translate-y-1 
        transition-all duration-300 relative overflow-hidden shadow-sm
      `,
      pill: 'rounded-xl border border-slate-200 shadow-sm',
        ...
        <div className={styles.header}>
            <div className="flex justify-between items-center mb-3 pt-4">
                ...
                <div className="flex gap-2">
                    <label className="w-10 h-10 rounded-2xl glass-icon text-emerald-600 active:scale-95 transition-all flex items-center justify-center cursor-pointer shadow-sm border border-slate-200 hover:shadow-md" title="استيراد Excel">
                            {isImporting ? <Loader2 className="w-5 h-5 animate-spin"/> : <FileUp className="w-5 h-5"/>}
                            <input type="file" ref={fileInputRef} onChange={handleImportExcel} accept=".xlsx, .xls" className="hidden" />
                    </label>
                    <button onClick={handleExportExcel} disabled={isExporting} className="w-10 h-10 rounded-2xl glass-icon text-indigo-600 active:scale-95 transition-all shadow-sm border border-slate-200 hover:shadow-md" title="تصدير Excel">
                        {isExporting ? <Loader2 className="w-5 h-5 animate-spin"/> : <FileSpreadsheet className="w-5 h-5"/>}
                    </button>
                    <button onClick={handleClearGrades} className="w-10 h-10 rounded-2xl glass-icon text-rose-600 active:scale-95 transition-all shadow-sm border border-slate-200 hover:shadow-md" title="حذف درجات الفصل الحالي">
                        <Trash2 className="w-5 h-5"/>
                    </button>
                    <button onClick={() => setShowToolsManager(true)} className="w-10 h-10 rounded-2xl glass-icon text-slate-500 active:scale-95 transition-all shadow-sm border border-slate-200 hover:shadow-md" title="إعدادات أدوات التقويم">
            ...
            <div className="space-y-2 mb-2">
                ...
                {availableGrades.length > 0 && (
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                        <button onClick={() => { setSelectedGrade('all'); setSelectedClass('all'); }} className={`px-4 py-2 text-[10px] font-bold whitespace-nowrap transition-all rounded-xl border ${selectedGrade === 'all' ? 'bg-indigo-600 text-white border-indigo-700 shadow-md' : 'bg-white border-slate-200 text-slate-500 shadow-sm'}`}>كل المراحل</button>
                        {availableGrades.map(g => (
                            <button key={g} onClick={() => { setSelectedGrade(g); setSelectedClass('all'); }} className={`px-4 py-2 text-[10px] font-bold whitespace-nowrap transition-all rounded-xl border ${selectedGrade === g ? 'bg-indigo-600 text-white border-indigo-700 shadow-md' : 'bg-white border-slate-200 text-slate-500 shadow-sm'}`}>صف {g}</button>
                        ))}
                    </div>
                )}
                ...
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                    {visibleClasses.map(c => (
                        <button key={c} onClick={() => setSelectedClass(c)} className={`px-5 py-2.5 text-xs font-bold whitespace-nowrap transition-all ${selectedClass === c ? 'bg-indigo-600 text-white shadow-indigo-200 border-indigo-700' : 'bg-white text-slate-500 hover:bg-slate-50'} ${styles.pill}`}>{c}</button>
                    ))}
                </div>
            </div>
            ...
            <div className="overflow-x-auto no-scrollbar flex gap-2 pt-1 pb-1">
                {tools.length > 0 ? tools.map(tool => (
                    <button 
                        ...
                        className="px-4 py-2 glass-card rounded-xl text-[10px] font-bold text-slate-600 whitespace-nowrap hover:bg-indigo-50 border border-slate-200 flex items-center gap-1.5 active:scale-95 shadow-sm transition-all"
                    >
                        ...
                        return (
                            <div key={student.id} onClick={() => setShowAddGrade({ student })} className={`${styles.card} p-4 flex items-center justify-between cursor-pointer active:scale-[0.99] group`}>
                                <div className="flex items-center gap-4 relative z-10">
                                    <div className="w-14 h-14 rounded-2xl bg-gray-50 flex items-center justify-center font-bold text-slate-400 overflow-hidden shadow-inner border border-gray-100 group-hover:border-indigo-100 transition-colors">
                                        {student.avatar ? <img src={student.avatar} className="w-full h-full object-cover"/> : student.name.charAt(0)}
                                    </div>
                                    <div>
                                        <h3 className="text-sm font-black text-slate-800 group-hover:text-indigo-700 transition-colors">{student.name}</h3>
                                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                            {tools.slice(0, 3).map(tool => {
                                                const grade = semGrades.find(g => g.category.trim() === tool.name.trim());
                                                return (
                                                    <span key={tool.id} className="text-[9px] px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-100 text-slate-500 font-bold">
                                ...
                                <div className="text-center bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 shadow-inner group-hover:bg-white transition-colors relative z-10 min-w-[70px]">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                        {tools.map(tool => (
                            <button 
                                ...
                                className={`p-3 rounded-xl text-xs font-black transition-all border ${selectedToolId === tool.id ? 'bg-indigo-600 text-white border-transparent shadow-md' : 'bg-white text-slate-500 border-slate-200 hover:bg-gray-50 shadow-sm'}`}
                            >
                                {tool.name}
                            </button>
                        ))}
                    ...
                    <div className="border-t border-gray-100 pt-4 mt-2">
                        ...
                        <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar p-1">
                            {getSemesterGrades(showAddGrade.student, currentSemester).length > 0 ? getSemesterGrades(showAddGrade.student, currentSemester).map(g => (
                                <div key={g.id} className="flex items-center justify-between p-3 rounded-xl bg-white border border-slate-100 hover:border-indigo-100 shadow-sm transition-colors">
                ...
                {!isAddingTool ? (
                    <>
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar p-1">
                            {tools.length > 0 ? tools.map(tool => (
                                <div key={tool.id} className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-200 shadow-sm group hover:border-indigo-200 transition-colors">
