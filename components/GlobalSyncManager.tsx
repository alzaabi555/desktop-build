...
// =========================================================================
// 🎮 تجهيز أسئلة الألعاب التعليمية لراصد الطالب
// =========================================================================
...
const GlobalSyncManager: React.FC = () => {
  ...
  // =========================================================================
  // 🎮 نشر أسئلة الألعاب التعليمية من راصد المعلم إلى راصد الطالب
  // =========================================================================
  const handlePublishGameQuestions = async (
    ...
    setSyncMessage('جاري نشر أسئلة الألعاب التعليمية للطلاب...');

    try {
      const studentGameQuestions = sanitizeGameQuestionsForStudent(
        payload.questions
      );

      // حفظ آخر نسخة منشورة محليًا حتى تُرسل مع المزامنة السريعة لاحقًا
      ...
      await fetch(STUDENT_APP_URL, {
        method: 'POST',
        body: JSON.stringify(gamesPayload)
      });

      setSyncState('success');
      setSyncMessage('تم نشر أسئلة الألعاب التعليمية بنجاح.');
      setTimeout(() => setSyncState('idle'), 3000);
    } catch (error) {
      console.error(error);
      setSyncState('error');
      setSyncMessage('تعذر نشر أسئلة الألعاب التعليمية.');
      setTimeout(() => setSyncState('idle'), 4000);
    }
  };

  const handleSync = async (
    type: 'student' | 'parent' | 'backup' | 'restore' | 'admin'
  ) => {
    // حماية السحابة المركزية
    if ((type === 'backup' || type === 'restore') && !teacherInfo?.civilId) {
      alert(
        'الرجاء إدخال رقمك المدني (كمعلم) في الإعدادات لربط نسختك الاحتياطية.'
      );
      return;
    }
    ...
    try {
      ...
      if (type === 'student') {
        ...
        const savedGameQuestions = JSON.parse(
          localStorage.getItem('rased_teacher_published_game_questions') ||
            '[]'
        );
      ...
      // 👨‍👩‍👦 2. تطبيق أولياء الأمور
        ...
        if (parentPayload.length === 0) {
          throw new Error(
            'لا يوجد أي طالب يمتلك كود راصد السري (RSD) للمزامنة!'
          );
        }

        await fetch(PARENT_APP_URL, {
          method: 'POST',
      ...
      // 🏫 3. تطبيق راصد الإدارة
        ...
        localStorage.setItem(
          'rased_admin_school_code',
          adminSchoolCode.trim()
        );
        ...
        students.forEach(s => {
          const todayRecord = s.attendance?.find(
            a =>
              a.date === todayCA ||
      ...
      // ☁️ 4. السحابة المركزية: رفع احتياطي
      else if (type === 'backup') {
        ...
        const recordsToSync = [
          {
            id: 'tools_data',
            type: 'Tools',
            data: JSON.stringify(assessmentTools),
          ...
          {
            id: 'categorizations_data',
            type: 'Categorizations',
            data: JSON.stringify(categorizations || []),
            lastUpdated: forceTimestamp
          },
          {
            id: 'gameQuestions_data',
            type: 'GameQuestions',
            data:
          ...
          },
          {
            id: 'periodTimes_data',
            type: 'PeriodTimes',
            data: JSON.stringify(periodTimes || []),
            lastUpdated: forceTimestamp
          },
          {
            id: 'certSettings_data',
            type: 'CertSettings',
            data: JSON.stringify(certificateSettings || {}),
            lastUpdated: forceTimestamp
          },
          {
            id: 'hiddenClasses_data',
            type: 'HiddenClasses',
            data: JSON.stringify(hiddenClasses || []),
            lastUpdated: forceTimestamp
          }
        ];
      ...
      // 📥 5. السحابة المركزية: استرجاع البيانات
        ...
        if (
          ...
          result.records.forEach((serverRec: any) => {
            ...
            try {
              ...
              if (serverRec.id === 'tools_data') {
                newAssessmentTools = parsedData;
              }

              if (serverRec.id === 'groups_data') {
                newGroups = parsedData;
              }

              if (serverRec.id === 'categorizations_data') {
                newCategorizations = parsedData;
              }

              if (serverRec.id === 'gameQuestions_data') {
                newGameQuestions = parsedData;
              }

              if (serverRec.id === 'gradeSettings_data') {
                newGradeSettings = parsedData;
              }

              if (serverRec.id === 'classes_data') {
                newClasses = parsedData;
              }

              if (serverRec.id === 'teacher_info_data') {
                newTeacherInfo = parsedData;
              }

              if (serverRec.id === 'schedule_data') {
                newSchedule = parsedData;
              }

              if (serverRec.id === 'periodTimes_data') {
                newPeriodTimes = parsedData;
              }

              if (serverRec.id === 'certSettings_data') {
                newCertificateSettings = parsedData;
              }

              if (serverRec.id === 'hiddenClasses_data') {
                newHiddenClasses = parsedData;
              }
          ...
          if (incomingChunks.length > 0) {
            incomingChunks.sort(
              (a, b) =>
                parseInt(a.id.replace('students_chunk_', '')) -
                parseInt(b.id.replace('students_chunk_', ''))
            );

            newStudents = incomingChunks.reduce(
              (acc, chunk) => acc.concat(chunk.data),
              []
            );
          ...
          if (hasData) {
            localStorage.setItem(
              'rased_teacher_published_game_questions',
              JSON.stringify(newGameQuestions || [])
            );
            ...
            const dataToSave = {
              ...
              gameQuestions: newGameQuestions
            };
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="rounded-xl border border-borderColor bg-bgCard p-4 shadow-sm transition-all hover:shadow-md">
                <p className="text-xs font-bold text-textSecondary mb-1">
                  عدد الطلاب
                </p>
                <h3 className="text-xl md:text-2xl font-black text-textPrimary">
                  {students.length}
                </h3>
              </div>
            ...
            <TeacherGameQuestionsManager
              ...
              teacherId={teacherInfo?.civilId || 'default_teacher'}
              ...
              <div className="md:col-span-2 rounded-3xl border border-borderColor bg-bgCard p-5 space-y-4 shadow-sm">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl border border-borderColor bg-bgSoft hover:bg-bgCard hover:shadow-md transition">
                  <div className="flex-1">
                    <h4 className="font-bold text-base text-textPrimary">
                      تطبيق الطلاب
                    </h4>
                    <p className="text-xs font-bold text-textSecondary mt-1">
                      إرسال المهام والدرجات وأسئلة الألعاب
                    </p>
                  </div>
                ...
                <div className="flex flex-col gap-3 p-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 hover:bg-emerald-500/10 transition">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-xs font-bold text-textSecondary mt-1">
                        إرسال تقرير الغياب والحضور للإدارة
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 mt-2">
                    <input
                      ...
                      onChange={e =>
                        setAdminSchoolCode(e.target.value.replace(/\D/g, ''))
                      }
                    ...
                    <button
                      ...
                      className="w-full sm:w-auto px-6 py-3 rounded-xl bg-emerald-500 text-white font-bold flex items-center justify-center gap-2 hover:bg-emerald-600 transition active:scale-95 shrink-0"
                    >
