  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        await Share.share({
          title: 'دليل راصد المعلم',
          text: 'دليل الاستخدام الشامل لراصد المعلم',
          url: result.uri,
          dialogTitle: 'حفظ أو مشاركة الدليل'
        });
      } else {
        await worker.save();
      }
      ...
      alert('تعذر تصدير دليل التطبيق إلى PDF. حاول مرة أخرى.');
    } finally {
          ...
          <div className="px-4 md:px-5 pb-5 border-t border-borderColor bg-bgSoft/40">
            <div className="pt-4">
              <p className="text-[11px] font-black text-textSecondary mb-3 flex items-center gap-2">
                <ListChecks size={15} className="text-primary" /> خطوات الاستخدام
              </p>
            ...
            {topic.tips?.map((tip, index) => (
              <div key={`${topic.id}-tip-${index}`} className="mt-4 rounded-2xl p-3 bg-info/10 text-info border border-info/20 text-xs font-bold leading-6 flex items-start gap-2">
                ...
                <span><strong>نصيحة:</strong> {tip}</span>
              </div>
            ))}
            ...
            {topic.warning && (
              <div className="mt-4 rounded-2xl p-3 bg-warning/10 text-warning border border-warning/20 text-xs font-bold leading-6 flex items-start gap-2">
                ...
                <span><strong>تنبيه:</strong> {topic.warning}</span>
              </div>
            )}
  ...
  return (
    <div className={`h-full min-h-0 bg-bgMain text-textPrimary ${dir === 'rtl' ? 'text-right' : 'text-left'}`} dir={dir}>
      <div className="h-full min-h-0 flex overflow-hidden">
        {isSidebarOpen && (
          <button type="button" aria-label="إغلاق القائمة" onClick={() => setSidebarOpen(false)} className="fixed inset-0 z-40 bg-black/30 lg:hidden" />
        )}
          ...
          <div className="h-full flex flex-col pt-[max(env(safe-area-inset-top),12px)]">
            <div className="p-4 border-b border-borderColor flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-black text-lg">دليل راصد المعلم</h2>
                  <p className="text-[10px] font-bold text-textSecondary">مركز المساعدة التفاعلي</p>
                </div>
              </div>
            ...
            <div className="p-3 border-b border-borderColor">
              <label className="relative block">
                ...
                <input value={query} onChange={event => setQuery(event.target.value)} placeholder="ابحث في الدليل..." className={`w-full h-11 rounded-2xl bg-bgSoft border border-borderColor text-sm font-bold outline-none focus:border-primary ${dir === 'rtl' ? 'pr-10 pl-3' : 'pl-10 pr-3'}`} />
              </label>
              {query && <p className="mt-2 text-[10px] font-black text-primary">{searchResultsCount} موضوع مطابق</p>}
            </div>
            ...
            <div className="p-3 border-t border-borderColor">
              <button type="button" onClick={handleDownloadPDF} disabled={isExporting} className="w-full h-11 rounded-2xl bg-primary text-white font-black text-xs flex items-center justify-center gap-2 disabled:opacity-60">
                ...
                {isExporting ? 'جاري إعداد الدليل...' : 'حفظ الدليل PDF'}
              </button>
            </div>
          </div>
        </aside>
          ...
          <div id="guide-print-content" className="max-w-7xl mx-auto px-4 md:px-6 py-5 md:py-8 pb-28">
            <header className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-primary via-primary to-indigo-700 text-white p-5 md:p-8 mb-5 md:mb-7 shadow-lg">
              ...
              <div className="relative z-10 grid grid-cols-1 xl:grid-cols-[1fr_auto] gap-6 items-center">
                <div>
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/15 border border-white/20 text-[10px] font-black mb-4"><Sparkles size={13} /> الدليل الشامل للإصدار الحديث</div>
                  <h1 className="text-3xl md:text-5xl font-black leading-tight mb-3">استخدم راصد المعلم بثقة واحتراف</h1>
                  <p className="text-sm md:text-base font-bold text-white/80 leading-7 max-w-3xl">دليل تفاعلي يغطي الإعداد والطلاب والجدول والخطط والمجموعات والمراسلات والمهام والمكتبة والألعاب والتقارير والنسخ الاحتياطية.</p>
                </div>
                <div className="grid grid-cols-3 gap-2 min-w-[260px]">
                  <div className="rounded-2xl bg-white/15 border border-white/20 p-3 text-center"><p className="text-2xl font-black">{sections.length}</p><p className="text-[9px] font-bold text-white/70">أقسام</p></div>
                  <div className="rounded-2xl bg-white/15 border border-white/20 p-3 text-center"><p className="text-2xl font-black">{totalTopics}</p><p className="text-[9px] font-bold text-white/70">موضوعًا</p></div>
                  <div className="rounded-2xl bg-white/15 border border-white/20 p-3 text-center"><p className="text-2xl font-black">3</p><p className="text-[9px] font-bold text-white/70">منصات</p></div>
                </div>
              </div>
            </header>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5 md:mb-7">
              ...
              <div className="bg-bgCard border border-borderColor rounded-3xl p-4 flex items-center gap-3"><Smartphone className="text-success" /><div><p className="font-black text-sm">الجوال والتابلت</p><p className="text-[10px] font-bold text-textSecondary">قوائم متجاوبة ومشاركة الملفات</p></div></div>
              <div className="bg-bgCard border border-borderColor rounded-3xl p-4 flex items-center gap-3"><ShieldCheck className="text-warning" /><div><p className="font-black text-sm">النسخ الآمن</p><p className="text-[10px] font-bold text-textSecondary">حفظ محلي وسحابي شامل</p></div></div>
            </section>

            {filteredSections.length === 0 ? (
              <div className="bg-bgCard border border-dashed border-borderColor rounded-3xl p-10 text-center"><FileQuestion className="w-14 h-14 mx-auto text-textMuted mb-3" /><h2 className="font-black text-lg">لا توجد نتائج</h2><p className="text-xs font-bold text-textSecondary mt-2">جرّب كلمة بحث مختلفة مثل: المجموعات، الشهادات، النسخة السحابية أو الألعاب.</p></div>
            ) : (
            ...
            <footer className="mt-8 rounded-3xl bg-bgCard border border-borderColor p-5 text-center">
              ...
              <p className="font-black text-sm">دليل راصد المعلم</p>
              <p className="text-[10px] font-bold text-textSecondary mt-1">تحديث شامل لميزات التطبيق الحديثة • {new Date().getFullYear()}</p>
            </footer>
          </div>
        </main>
      </div>
