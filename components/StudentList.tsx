...
import {
    ...
    Sparkles, Send, Loader2, Mail, RefreshCcw, Printer, Reply
} from 'lucide-react';
...
// -------------------------------------------------------------------------
// الهدف:
// - عدم الاعتماد على الاسم فقط عند الاستيراد.
// - منع إضافة طالب جديد إذا كان نفس الاسم ونفس الفصل موجودين.
// - الحفاظ على كود RSD القديم عند الدمج.
// =========================================================================
        ...
        >
                                ...
                                <div key={student.id} className="bg-bgCard print:bg-white border-2 border-dashed border-borderColor print:border-gray-400 p-5 rounded-2xl flex flex-col items-center text-center shadow-sm relative overflow-hidden">
                                    ...
                                    <div className="relative z-10 w-full">
                                        ...
                                        <div className="mt-5 mb-4 p-4 bg-primary/5 print:bg-gray-50 border border-primary/20 print:border-gray-300 rounded-xl w-full">
                                            ...
                                            <p className="font-mono text-xl md:text-2xl font-black text-primary print:text-black tracking-widest">{(student as any).rasedId || (student as any).parentCode || 'جاري التوليد...'}</p>
                                        </div>
                    ...
                    <ExcelImport
                        ...
                        onImport={(importedStudents) => {
                            setStudents(prevStudents => {
                                const updatedStudents: Student[] = [...prevStudents];

                                importedStudents.forEach((imported: Student) => {
                                    const importedClass = getStudentClassValueForIdentity(imported);
                                    const existingIndex = updatedStudents.findIndex(existing => {
                                        return makeStudentIdentityKey(existing.name || '', getStudentClassValueForIdentity(existing)) ===
                                            makeStudentIdentityKey(imported.name || '', importedClass);
                                    });

                                    if (existingIndex >= 0) {
                                        updatedStudents[existingIndex] = mergeStudentKeepingOldCode(updatedStudents[existingIndex], imported);
                                    } else {
            ...
            <DrawerSheet isOpen={showManageClasses} onClose={() => setShowManageClasses(false)} isRamadan={isRamadan} dir={dir}>
                <div className="flex flex-col h-full w-full text-center">
                    ...
                    <div className="w-full h-px mb-6 shrink-0 bg-borderColor" />
            ...
            <DrawerSheet isOpen={showPositiveModal} onClose={() => { setShowPositiveModal(false); setSelectedStudentForBehavior(null); }} isRamadan={isRamadan} dir={dir}>
                <div className="flex flex-col h-full w-full text-center pb-4">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar px-1">
                        <div className="grid grid-cols-2 gap-2 mb-4">
                            {POSITIVE_BEHAVIORS.map(b => (
                                <button key={b.id} onClick={() => confirmPositiveBehavior(b.original, b.points)} className="p-3 border rounded-xl text-xs font-bold active:scale-95 transition-all flex flex-col items-center gap-1 bg-emerald-500/10 border-emerald-500/30 text-emerald-600 hover:bg-emerald-500/20">
                    ...
                    <div className="pt-3 border-t shrink-0 mt-auto border-borderColor">
                        ...
                        <div className="flex gap-2">
                            <input type="text" value={customPositiveReason} onChange={(e) => setCustomPositiveReason(e.target.value)} placeholder={t('otherReasonPlaceholder')} className="flex-1 border rounded-lg px-3 py-2 text-xs font-bold outline-none transition-colors bg-bgCard border-borderColor focus:border-emerald-500 text-textPrimary" />
                            <button
                                onClick={() => {
            ...
            <DrawerSheet isOpen={showNegativeModal} onClose={() => { setShowNegativeModal(false); setSelectedStudentForBehavior(null); }} isRamadan={isRamadan} dir={dir}>
                <div className="flex flex-col h-full w-full text-center pb-4">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar px-1">
                        <div className="grid grid-cols-2 gap-2 mb-4">
                            {NEGATIVE_BEHAVIORS.map(b => (
                                <button key={b.id} onClick={() => confirmNegativeBehavior(b.original, b.points)} className="p-3 border rounded-xl text-xs font-bold active:scale-95 transition-all flex flex-col items-center gap-1 bg-rose-500/10 border-rose-500/30 text-rose-600 hover:bg-rose-500/20">
                                    <span>{t(b.transKey)}</span>
                    ...
                    <div className="pt-3 border-t shrink-0 mt-auto border-borderColor">
                        ...
                        <div className="flex gap-2">
                            <input type="text" value={customNegativeReason} onChange={(e) => setCustomNegativeReason(e.target.value)} placeholder={t('otherReasonPlaceholder')} className="flex-1 border rounded-lg px-3 py-2 text-xs font-bold outline-none transition-colors bg-bgCard border-borderColor focus:border-rose-500 text-textPrimary" />
                            <button
                                onClick={() => {
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        <div className="grid grid-cols-3 gap-2 mb-6">
                            {[1, 3, 5, 10, 15, 20].map(min => (
                                <button key={min} onClick={() => startTimer(min)} className="border rounded-xl py-3 text-xs font-bold transition-all active:scale-95 bg-transparent border-borderColor text-textSecondary hover:bg-primary/10 hover:border-primary/30 hover:text-primary">
                                    {min} {t('minuteAbbrev')}
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="flex gap-2 items-center mb-6">
                            <input type="number" value={timerInput} onChange={(e) => setTimerInput(e.target.value)} className="w-full border rounded-xl py-3 px-3 text-center font-black outline-none transition-colors bg-transparent border-borderColor focus:border-primary text-textPrimary placeholder:text-textSecondary" placeholder={t('minutePlaceholder')} />
                            <button onClick={() => startTimer(Number(timerInput))} className="p-3.5 rounded-xl active:scale-95 shadow-lg transition-colors bg-primary hover:bg-primary/80">
