...
import { Search, Edit2, Trash2, Plus, LayoutGrid, Settings, UserPlus, Upload, Sparkles, X, Trophy, Frown, CloudRain, PartyPopper, Check, Users, Dices } from 'lucide-react';
...
const SOUNDS = {
    ...
    tada: 'https://assets.mixkit.co/active_storage/sfx/1435/1435-preview.mp3' // ✅ تم إضافة صوت الاحتفال
};
...
const StudentList: React.FC<StudentListProps> = ({ students, classes, onAddClass, onAddStudentManually, onBatchAddStudents, onUpdateStudent, onDeleteStudent, onViewReport, currentSemester, onDeleteClass }) => {
    ...
    // =========================================================================
    // ✅ 2. دالة القرعة الذكية (Smart Random Logic) - مستبدلة بالكامل
    // =========================================================================
    const [randomWinner, setRandomWinner] = useState<Student | null>(null);
    const [pickedStudentIds, setPickedStudentIds] = useState<string[]>([]); // ذاكرة لتجنب التكرار

    // إعادة تعيين القائمة عند تغيير الفصل (بدء حصة جديدة)
    useEffect(() => {
        setPickedStudentIds([]);
    ...
    const handleRandomPick = () => {
        ...
        // 1. تصفية الطلاب: استبعاد الغياب والتسرب من قائمة الطلاب الظاهرين حالياً
        ...
        if (eligibleCandidates.length === 0) {
            ...
            if (confirm('تم اختيار جميع الطلاب الحاضرين. هل تريد بدء دورة جديدة؟')) {
                setPickedStudentIds([]); // إعادة تعيين القائمة
            }
            return;
        }

        // 3. الاختيار العشوائي الفوري
        const randomIndex = Math.floor(Math.random() * eligibleCandidates.length);
        const winner = eligibleCandidates[randomIndex];

        // 4. تحديث الحالة وتشغيل الصوت
        setPickedStudentIds(prev => [...prev, winner.id]);
        setRandomWinner(winner);
        
        // تشغيل صوت الاحتفال
        const audio = new Audio(SOUNDS.tada);
        audio.volume = 0.5;
    ...
    return (
        <div className="flex flex-col h-full text-slate-800 relative bg-[#f3f4f6]">
                            ...
                            <>
                                ...
                                <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden animate-in fade-in zoom-in duration-200 origin-top-left text-slate-800">
                                    <div className="flex flex-col py-1">
                                        ...
                                        {/* ✅ زر القرعة مرتبط بالدالة الجديدة */}
                                        <button onClick={() => { handleRandomPick(); setShowMenu(false); }} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 text-right w-full font-bold text-sm border-t border-slate-50 text-slate-700">
            ...
            {/* ... Modals (Add, Import, Settings) ... */}
            {/* تم اختصار المودالات المتكررة هنا، ولكن في الكود الفعلي يجب أن تبقى موجودة كما كانت */}
                ...
                <div className="text-center text-slate-900">
                    ...
                    <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 mb-6 shadow-sm">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => handleBulkGenderUpdate('male')} className="flex-1 bg-white py-3 rounded-xl border border-indigo-100 shadow-sm text-sm font-bold text-slate-700 hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center justify-center gap-2"><span className="text-lg">👦</span> بنين</button>
                            <button onClick={() => handleBulkGenderUpdate('female')} className="flex-1 bg-white py-3 rounded-xl border border-indigo-100 shadow-sm text-sm font-bold text-slate-700 hover:bg-pink-50 hover:text-pink-600 transition-all flex items-center justify-center gap-2"><span className="text-lg">👧</span> بنات</button>
                        </div>
                        <p className="text-[9px] text-indigo-400 mt-2 text-right font-bold opacity-70">* سيتم توحيد أيقونات جميع الطلاب حسب اختيارك.</p>
                    </div>
            ...
            {/* ✅ نافذة الفائز في القرعة (تم تحديثها لعرض الفائز بشكل احتفالي) */}
            <Modal isOpen={!!randomWinner} onClose={() => setRandomWinner(null)} className="max-w-sm rounded-[2rem]">
                {randomWinner && (
                    <div className="text-center py-6 animate-in zoom-in duration-300">
                        ...
                        {randomWinner.classes && randomWinner.classes[0] && (
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => { handleAddBehavior(randomWinner, 'positive', 'اختيار عشوائي', 1); setRandomWinner(null); }} className="flex-1 py-3 bg-emerald-500 text-white rounded-xl font-black text-sm shadow-lg shadow-emerald-200 active:scale-95 transition-all">
                                تعزيز (+1)
                            </button>
                            <button onClick={() => setRandomWinner(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-xl font-bold text-sm hover:bg-slate-200 transition-all">
                                إغلاق
                            </button>
                        </div>
                    </div>
